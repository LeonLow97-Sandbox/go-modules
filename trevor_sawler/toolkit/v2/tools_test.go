	...
	for _, e := range uploadTests {
		...
		if !e.errorExpected {
			if _, err := os.Stat(fmt.Sprintf("./testdata/uploads/%s", uploadedFiles[0].NewFileName)); os.IsNotExist(err) {
				t.Errorf("%s: expected file to exist: %s", e.name, err)
			}
...
var jsonTests = []struct {
	...
	{name: "unknown field in json", json: `{"fooooooooo": "1"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allow unknown fields in json", json: `{"fooooooooo": "1"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
...
func TestTools_ReadJSON9(t *testing.T) {
	var testTool Tools
